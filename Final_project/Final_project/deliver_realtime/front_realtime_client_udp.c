...
void init_SDL(SDL_Window **window, SDL_Renderer **renderer, SDL_Texture **texture) {
    ...
    // 創建窗口
    *window = SDL_CreateWindow("Front Client Camera", INIT_WINDOW_POS_X, INIT_WINDOW_POS_Y, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_OPENGL);
    if (!*window) {
...
void sendAlarm111ToServer() {
    ...
    // 向服务器发送请求以开始接收数据
    strcpy(tcp_buf, "111"); // back傳送01給server
    if (send(tcp_sockfd, tcp_buf, strlen(tcp_buf), 0) < 0) {
        perror("Error sending to server");
        close(tcp_sockfd);
        exit(1);
