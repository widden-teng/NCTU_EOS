...
while (1):
    ...
    if(camera == True):
        ...
        cv2.moveWindow(window_name, 100, 100)  # 視窗的 x, y 位置
        count = 0
        ...
        if(bad_guy_ratio > 0.6):
            bad_guy_flag = True
            file_name = "./deliver_photo/front_camera.jpg"
            cv2.imwrite(file_name, show_img)
        ...
        if(bad_guy_flag):
            ...
            # 開啟UDP server
            subprocess.Popen(["./deliver_realtime/front_realtime_server_udp"])
            print("!!!!!!!!!!")
            bad_guy_flag = False
        else:
            # cap.release()
